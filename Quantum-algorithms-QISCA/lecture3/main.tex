...
\begin{frame}
  ...
  \begin{itemize}
    \item If no item is marked, step 4 never fires --- the algorithm never terminates by itself.
    \item Step 5 caps the window at $m = \sqrt{N}$, so the ramp-up gets stuck there. After that, every loop pass simply runs at the maximum window size.
    \item At $m = \sqrt{N}$, \emph{if} there were even one marked item ($t \ge 1$), each pass would still find it with probability $\ge 1/4$.
    \item So we wrap the algorithm in a \textbf{time-out}: run a constant number $r$ of passes at $m = \sqrt{N}$. If none returns a marked item, declare ``$t = 0$''.
    \item The false-negative probability is at most $(3/4)^r$ --- exponentially small in $r$.
  \end{itemize}

  \vspace{0.4cm}
  \textbf{This is exactly how D\"urr--H\o yer detects ``$y$ is already the minimum''.}
\end{frame}
