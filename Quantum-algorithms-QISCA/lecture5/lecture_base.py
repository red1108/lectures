...
ROADMAP_POSITIONS = (0.06, 0.19, 0.34, 0.49, 0.63, 0.78, 0.93)
...
def roadmap_step_for(major_section: str, section: str) -> str:
    ...
    if section in {"", "Question", "Library"}:
        return "Hook"
    if section in {"One pick", "k-NN"}:
        return "k-NN"
    if section == "Swap Test":
        return "Swap"
    if section == "Baseline":
        return "Base"
    if section in {"Coherent", "QADC"}:
        return "QADC"
    if section in {"Comparator", "Top-k"}:
        return "Compare"
    if section in {"Primitives", "Ranking", "Collision", "Closing"}:
        return "Tools"
    return major_section if major_section in ROADMAP_STEPS else "Hook"
